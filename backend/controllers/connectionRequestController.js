...
const sendConnectionRequest = asyncHandler(async (req, res) => {
  ...
  if (!receiverId) {
    return res
      .status(400)
      .send({ success: false, message: "Receiver ID is required" });
  }
  ...
  await User.findByIdAndUpdate(senderId, {
    $push: { connectionRequestsSent: receiverId },
  });
  await User.findByIdAndUpdate(receiverId, {
    $push: { connectionRequestsReceived: senderId },
  });
...
const acceptConnectionRequest = asyncHandler(async (req, res) => {
  ...
  await User.findByIdAndUpdate(request.sender, {
    $push: { connections: request.receiver },
  });
  ...
  await User.findByIdAndUpdate(request.sender, {
    $pull: { connectionRequestsSent: request.receiver },
  });
  await User.findByIdAndUpdate(request.receiver, {
    $pull: { connectionRequestsReceived: request.sender },
  });
...
const getUserConnections = asyncHandler(async (req, res) => {
  ...
  if (!user || !user.connections) {
    return res
      .status(404)
      .send({ success: false, message: "No connections found" });
  }
